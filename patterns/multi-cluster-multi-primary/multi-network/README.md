...
In this setup the we would have two different VPCs (multi-network) in which we 
would create two EKS clusters one in each VPC. Istio would be installed on each 
of the EKS clusters as primary (multi-primary). 
...
✅ **Note:** The EKS clusters created for this usecase use [EKS Auto Mode](https://docs.aws.amazon.com/eks/latest/userguide/automode.html)
